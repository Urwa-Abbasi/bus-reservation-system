...
function confirmBooking() {
    ...
    if (!currentUser || !currentUser.id) return alert('⚠️ You must be logged in');
    ...
    .then(data => {
        if (data.success) {
            ...
            let all = JSON.parse(localStorage.getItem('reservations') || '[]');
            all.push(booking);
...
function loadMyReservations() {
    ...
    const my = (currentUser && currentUser.email) ? reservations.filter(r => r.passenger_email === currentUser.email) : reservations;
